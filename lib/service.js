...
Service.prototype.discoverCharacteristics = function(characteristicUuids, callback) {
  if (callback) {
    const listener = (characteristics) => {
      const result = characteristics.filter(c => characteristicUuids.indexOf(c.uuid) > -1)
      if (result.length > 0) {
        this.removeListener('characteristicsDiscover', listener)
